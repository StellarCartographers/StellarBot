...
public class RegisterListener extends ListenerAdapter
{
    ...
    {
        if(event.getButton().equals(MessageButtons.REGISTER_CARRIER.getButton()))
        {
            ...
            event.replyEmbeds(this.getConsentEmbed()).addActionRow(MessageButtons.CONSENT.getButton(), MessageButtons.DENY.getButton()).setEphemeral(true).queue();
            ...
            waiter.waitForEvent(ButtonInteractionEvent.class, p -> p.getMember().equals(member), action -> {
                ...
                eventWaiter.waitForEvent(AuthorizedEvent.class, done -> {
                    ...
                    var carrierResult = StellarAPI.getFleetCarrier(memberId, true);
                    if (carrierResult.isErr()) {
                        ...
                        Embed embed = Embed.newBuilder().description("There was an error during carrier validation, please report this to staff")
                                        .field("Error Encountered", CodeBlock.style("json").line(errState.toJson()).toString()).color(RGB.RED);
                    ...
                    if (createResult.isErr()) {
                        ...
                        hook.editOriginalEmbeds(Embed.newBuilder()
                                        .title("Validation Complete")
                                        .description("## Creating Fleet Carrier Channels")
                                        .footer("This message can be dismissed")
                                        .color(RGB.GREEN).toEmbed()
                        ).setComponents().queue();
                    }
                                ...
                                ownerChannel -> {
                                    ownerChannel.getPermissionContainer().getManager().putMemberPermissionOverride( 
                                        ...
                                        TSCGDatabase.instance().create(carrierManager).after(i -> {
                                            new CreatedCarrierEvent(carrier.getCarrierId(), memberId, event.getGuild().getId(), ownerChannel.getId(), publicChannel.getId()).post();
                                        });
                                    });
                                }
                            );
    ...
    private MessageEmbed getConsentEmbed()
    {
        return Embed.newBuilder()
                        .title("Data Useage Requirements")
                        .description("""
                                     ```asciidoc
    ...
    {
        if (MessageButtons.DENY.equals(event.getButton())) {
        ...
        if (MessageButtons.CONSENT.equals(event.getButton())) {
            var info = Embed.descriptionEmbed("This would normally be the OAuth link. Which would take you to Frontiers auth login, "
                            + "this mocks the actions that happen on the backend when the API Server receives a sucessfull authorization. If sucessfull, "
                            + "the user is directed to a \"authenticated\" landing page (https://tscg.network/authenticated)", RGB.GREEN);
            event.editMessageEmbeds().setEmbeds(info.toEmbed())
                .setComponents(ActionRow.of(MessageButtons.MOCK_FRONTIER.getButton())).queue();
        }
    }
}
